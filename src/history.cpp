...
int History::getContinuationHistory(const PieceType piece, const Move move, const int ply, const SearchStack *stack) const {
...
void History::updatePawnCorrectionHistory(const int bonus, const Board &board, const int div)
{
    ...
    // Gravity
    const int scaledBonus = bonus - pawnCorrectionHistory[board.sideToMove()][pawnHash & (pawnCorrectionHistorySize - 1)] * std::abs(bonus) / div;
...
int History::correctEval(const int rawEval, const Board &board) const {
    const int pawnEntry = pawnCorrectionHistory[board.sideToMove()][getPieceKey(PieceType::PAWN, board) & (pawnCorrectionHistorySize - 1)];
